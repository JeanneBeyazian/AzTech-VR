...
namespace Ubiq.Logging.Utf8Json
{
    public struct JsonWriter : IDisposable
    {
        ...
        {
            return new Span<byte>(buffer, 0, offset);
        }
        ...
        {
            var max = StringEncoding.UTF8.GetMaxByteCount(value.Length);
            BinaryUtil.EnsureCapacity(ref buffer, offset, max);
            offset += StringEncoding.UTF8.GetBytes(value, 0, value.Length, buffer, offset);
        }
        ...
        public void Write<T>(string propertyName, T property)
        {
            ...
            JsonSerializer.Serialize(ref this, property);
